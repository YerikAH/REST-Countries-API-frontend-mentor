...
const countryData= reactive<DataSuccess>({
  data: [],
  error: {
    status: 0,
...
const getCountryData = async (query: string) => {
  countryData.loading = true
  try {
    const dataCountry = (await apiService.getCountryData(query)) as Country[]
    countryData.data = dataCountry
  } catch (err) {
    const errorCountry = err as DataError
    countryData.error = errorCountry
  } finally {
    countryData.loading = true
  }
}
...
export { theme, changeTheme, data, getAllData, getSearchData, filterByRegion, getCountryData, countryData  }
